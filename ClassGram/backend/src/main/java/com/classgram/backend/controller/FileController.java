...
import com.classgram.backend.file.FileOperationsService;
import com.classgram.backend.file.MimeTypes;
...
public class FileController extends SecureController {
    ...
    public FileController(FileGroupService fileGroupService, FileService fileService, CourseService courseService,
                          CommentService commentService, UserService userService, UserComponent user, AuthorizationService authorizationService,
                          FileOperationsService fileOperationsService, CourseDetailsService courseDetailsService, EntryService entryService,
                          NotificationDispatcher notificationDispatcher, TelegramChannelService telegramChannelService) {
    ...
    public ResponseEntity<Object> handleFileUpload(@PathVariable(value = "courseId") String courseId, @PathVariable(value = "fileGroupId") String fileGroupId, @RequestParam("file") MultipartFile file, @PathVariable("file_type") int fileType)
        ...
        if (teacherAuthorized != null) { // If the user is not the teacher of the course
            ...
            com.classgram.backend.model.File customFile = new com.classgram.backend.model.File(fileType, fileName);
            ...
            TelegramChannel telegramChannel = c.getTelegramChannel();
            if (telegramChannel != null) {
                telegramChannelService.sendFileToTelegramChannel(telegramChannel.getDiscussionGroupId(), uploadedFile.getPath());
            }
    ...
    public void handleFileDownload(@PathVariable String fileId, @PathVariable(value = "courseId") String courseId,
        ...
        if (userAuthorized != null) { // If the user is not an attender of the course
            ...
            com.classgram.backend.model.File f = fileService.getFromId(id_file);
        ...
        if (userAuthorized != null) { // If the user is not an attender of the course
            ...
            if (commentParent != null) {
                ...
                while (i.hasNext()) {
                    ...
                    log.info("File succesfully uploaded to path '{}'", uploadedFile.getPath());
                    TelegramChannel telegramChannel = courseDetails.getCourse().getTelegramChannel();
                    if (telegramChannel != null) {
                        telegramChannelService.sendAudioMessageToTelegramChannel(telegramChannel.getDiscussionGroupId(), entry.getTelegramThreadChannel().getThreadChannelId(), uploadedFile.getPath());
                    }
                }
