...
public class VideoSessionController {
    ...
    public VideoSessionController(SessionService sessionService, AuthorizationService authorizationService, UserComponent user,
                                  NotificationDispatcher dispatcher, TelegramChannelService telegramChannelService) {
        ...
        if (session != null) { // sessionId belongs to a real Session
            ...
            if (this.lessonIdSession.get(id_i) == null) { // First user connecting to the session (only the teacher can)
                if (teacherAuthorized != null) { // If the user is not the teacher of the course
                    ...
                    TelegramChannel telegramChannel = session.getCourse().getTelegramChannel();
                    if (telegramChannel != null) {
                        telegramChannelService.sendTextMessageToTelegramChannel(telegramChannel.getChannelId(),
                                "Розпочалася відеоконференція!\nТема конференції: \"" + session.getTitle() + "\"" +
                                        String.format("\nПосилання на конференцію: %s", baseUrl));
                    }
