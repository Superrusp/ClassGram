...
import com.classgram.backend.annotation.CourseAuthorized;
import com.classgram.backend.annotation.RoleFilter;
import com.classgram.backend.model.Course;
import com.classgram.backend.model.Course.SimpleCourseList;
...
public class CourseController extends SecureController {
    ...
    public ResponseEntity<?> newCourse(@RequestBody Course course) {
        ...
        TelegramChannel telegramChannel = createdCourse.getTelegramChannel();
        if (telegramChannel != null) {

            telegramChannelService.sendTextMessageToTelegramChannel(telegramChannel.getChannelId(), "Створено канал для курсу " + "\"" + course.getTitle() + "\"."
                    + String.format("\nПосилання на курс: %s" + "courses/%s", baseUrl, course.getId()));
        }
...
//            telegramChannelService.updateChannelDetails(telegramChannelService.channel.getId(), telegramChannelService.channel.getAccessHash(), course.getCourseDetails().getInfo());
